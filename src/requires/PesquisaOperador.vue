<template>
  <!-- Modal Pesquisa Operador-->
  <div class="modal fade" id="modalPesquisaOperador" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabelOperador" aria-hidden="true">
    <div class="modal-dialog  modal-dialog-centered modal-lg">
      <div class="modal-content">
        <div class="modal-header" style="background-color: #003366; color:white; font-weight: bold;">
            <label class="tituloPesquisa fs-5" id="staticBackdropLabelOperador">Pesquisa de Operadores</label>
        </div>
        <div class="modal-body" >
          <div class="mb-3">
              <div class="col-md-12 input-group">
                  <input type="text" class="form-control form-control-sm" @blur="apiPesquisaNome('operadorCpf','nome',nomePesquisa,1)" v-model.lazy="nomePesquisa" placeholder="Informe o nome do operador">
                  <button type="button" class="btn btn-primary" @click="apiPesquisaNome('operadorCpf','nome',nomePesquisa,1)"><i class="bi bi-search input-group-append"></i></button>
              </div>
          </div>
              ...
              <thead>
                ...
                <tbody class="detalhePesquisa">
                    ...
                    <tr v-for="c in apiDisplayedDados" :key="c">
                        <th scope="row" class="text-center" style="color: blue; cursor: pointer;" @click="setaPesquisa({ tipo: 'operador',obj: c})" data-bs-dismiss="modal">{{ c.codigo }}</th>
...
<script>
    ...
    $('#modalPesquisaOperador').on('shown', function () {
        alert("aqui");
    })

    export default {
        name: 'PesquisaOperador',
        mixins: [ApiMixin],
        data: () => ({
            haErros : false,
            mensagemErro: '',
        ...
        methods:{
            ...
            fecharModal() {
                ...
                this.resultPesquisa = []
                this.pages = []
                $('#modalPesquisaOperador').modal('hide');
            },
        },
        watch: {
            haErros(newValue) {
